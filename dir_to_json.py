...
PARTIAL_READ_EXTENSIONS = {'.md', '.txt', '.log', '.properties', '.toml', '.kts', '.xml', '.java', '.kt'}
...
EXPLICIT_IGNORE_FILES = {
    ...
    # Wrapper scripts are noise for content-dumps
    'gradlew',
    'gradlew.bat',

    # Never include binaries
    'gradle-wrapper.jar',
}
...
EXPLICIT_IGNORE_PATTERNS = {
    ...
    '*.apk', '*.aab', '*.aar', '*.jar', '*.class', '*.dex', '*.so', '*.o',

    # Images / media (icons etc. bloat output)
...
EXPLICIT_IGNORE_FOLDERS = {
    ...
    # Android native build intermediates
    '.cxx',
    '.externalNativeBuild',
}
...
def is_explicitly_ignored(file_name):
    """Return True if filename matches any explicit ignore pattern."""
    for pattern in EXPLICIT_IGNORE_PATTERNS:
...
def dir_to_json(directory, submodules, ignore_submodules=False):
    ...
    for root, dirs, files in os.walk(directory):
        ...
        for file in files:
            if file in EXPLICIT_IGNORE_FILES or file in EXPLICIT_IGNORE_FOLDERS or is_explicitly_ignored(file):
                continue
